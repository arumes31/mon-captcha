...
async function measure(seed, nochunk) {
  ...
  const r = await p.evaluate(()=>{
    ...
    // direct scene render (standard shaders only, NOT the slow composer) so
    // renderer.info.render.calls reflects a real full-scene + shadow draw
    try { s.renderer.render(s.scene, s.camera); } catch(e){}
    try { P.updateCulling(); } catch(e){}
    try { s.renderer.render(s.scene, s.camera); } catch(e){}
    const samp = P.perf.sample();
    return { total:samp.totalInstances, drawn:samp.drawnInstances, totalMeshes:samp.totalMeshes,
             drawnMeshes:samp.drawnMeshes, occluded:samp.counters.occludedChunks, calls:samp.info.calls, tris:samp.info.triangles };
  });
  await ctx.close();
...
console.log("seed        mode      total  drawn  cut%   meshes(d/t)  occl  drawCalls  tris      err");
for (const r of rows){
